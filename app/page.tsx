...
// Tight translucent backdrop used on text blocks so the ASCII art behind
// stays visible but doesn't compromise readability.
const glass =
    ...
    <>
      ...
      <section className="pt-20 pb-16 sm:pt-28 sm:pb-20">
        ...
        <h1 className="text-3xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100 sm:text-4xl">
          <span className={glass}>Frontend engineer building</span>
          <br className="hidden sm:block" />{" "}
          <span className={glass}>interfaces that work.</span>
        </h1>
        <p
          className={`mt-4 max-w-xl text-lg leading-relaxed text-zinc-500 dark:text-zinc-400 ${glass}`}
        >
      ...
      {/* Focus areas */}
      <Section title="What I focus on" glassTitle>
        <ul className="grid gap-6 sm:grid-cols-3">
          {focus.map((item) => (
            <li key={item.title}>
              <h3
                className={`text-sm font-semibold text-zinc-900 dark:text-zinc-100 ${glass}`}
              >
                {item.title}
              </h3>
              <p
                className={`mt-1.5 text-sm leading-relaxed text-zinc-500 dark:text-zinc-400 ${glass}`}
              >
                {item.description}
              </p>
            </li>
      ...
      >
        ...
        <div className="mt-10">
          <Link
            href="/cases"
            className={`text-sm font-medium text-zinc-900 dark:text-zinc-100 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors ${glass}`}
          >
